  ...
  computed: {
    ...
    setCircleTrailStyle() {
      ...
      return `stroke-dasharray: ${len - (gapDeg || 0)}px, ${len}px;
				...
				transition: stroke-dashoffset 0.3s ease 0s, stroke-dasharray 0.3s ease 0s, stroke 0.3s;stroke:${
          this.strokeColor2 || "#ffffff"
        }`;
    },
    /**
     * 圈进度
    ...
    setCircle() {
      ...
      let svgToBase = `data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100' class='cmd-progress-circle'%3E%3Cpath d='${
        this.setCirclePath
      }' stroke='%23f3f3f3' stroke-linecap='${
        this.strokeShape
      }' stroke-width='${
        this.strokeWidth
        ...
        this.strokeWidth
      }' fill-opacity='0' class='cmd-progress-circle-path' style='stroke: ${escape(
        currentColor
      )};stroke-dasharray: ${
        (this.setProgress / 100) * (len - (gapDeg || 0))
      }px, ${len}px;stroke-dashoffset: -${
...
.cmd-progress-line {
  ...
  align-items: center;
  box-shadow: 2rpx 2rpx 4rpx 2rpx rgba(0, 0, 0, 0.05);
...
.cmd-progress-status-active .cmd-progress-bg:before {
  ...
  background: #fff;
  border-radius: 20upx;
  -webkit-animation: cmd-progress-active 2.4s cubic-bezier(0.23, 1, 0.32, 1)
    infinite;
